(function() {
  ...
  var ctx = canvas.getContext('2d');//コンテキストオブジェクトを変数へ格納
  ...
  var bl = { //blocksの略
    ...
    initMap: function() {
      for (var i = 0; i < this.cols; i++) { //アイテムをランダムに配置
        ...
        for (var j = 0; j < this.rows; j++) {
          ...
          this.map[i][j] = util.rand(4);//マップに数字を格納
        }
      }
    ...
    //ブロックを描画する関数
    blockDrow: function() {
      if (this.map.length){//map配列にデータがあれば
        for (var i = 0; i < this.cols; i++) {
          for (var j = 0; j < this.rows; j++) {
            if (this.map[i][j]) {//ブロックがあれば
              util.drawRect(//ブロックを描画
                this.width * j,//x座標を指定
                this.height * i + this.BLOCK_MARGIN_TOP,//y座標を指定
                this.width,//ブロックの幅
                this.height,//ブロックの高さ
                'rgb(' + this.colorMap[i][j][0] + ', ' + this.colorMap[i][j][1] + ', ' + this.colorMap[i][j][2] + ')'//ブロックの色を指定
              );
            }
          }
        }
  ...
  function ballHitsWalls() { //ボールが壁に当たった際に反射させる関数
    ...
    if (ballBottom > ctx.canvas.height) {//もしボールの中心位置が、canvasの底辺の位置まで来たら
      gameOver();//ゲームを終了する
    }
